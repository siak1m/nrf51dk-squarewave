 ...
 */
static void adc_event_handler_original(nrf_drv_adc_evt_t const * p_event)
{
...
static void adc_event_handler(nrf_drv_adc_evt_t const * p_event)
{
    ...
    if (p_event->type == NRF_DRV_ADC_EVT_DONE)
    {
        ...
        for (i = 0; i < p_event->data.done.size; i++)
        {
            printf("Sample value %d: %d\r\n", (i%number_of_adc_channels), p_event->data.done.p_buffer[i]);   //Print ADC result on hardware UART
            adc_result[(i*2)] = p_event->data.done.p_buffer[i] >> 8;
            adc_result[(i*2)+1] = p_event->data.done.p_buffer[i];					
        }
        if(ADC_BUFFER_SIZE <= 10)
 ...
 */
static void adc_config_original(void)
{
    ret_code_t ret_code;
...
static void adc_config(void)
{
	...
	//Configure and enable ADC channe 2
	static nrf_drv_adc_channel_t m_channel_2_config = NRF_DRV_ADC_DEFAULT_CHANNEL(NRF_ADC_CONFIG_INPUT_4);
